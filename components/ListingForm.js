...
const EMPTY = { title: "", category: "furniture", condition: "excellent", price: "", city: "Lagos", neighborhood: "", image: "", description: "" };
...
export default function ListingForm({ listingId }) {
  ...
  useEffect(() => {
    ...
    if (listingId) {
      ...
      if (!existing || existing.sellerId !== s.id) { router.replace("/seller/dashboard"); return; }
      setForm({
  ...
  function onSubmit(e) {
    ...
    const image = form.image.trim() || "https://images.unsplash.com/photo-1484101403633-562f891dc89a?auto=format&fit=crop&w=1400&q=80";
    const id = saveListing({ id: listingId, ...form, price: Number(form.price), image }, session.id);
    router.push(`/item/${id}`);
  }
  ...
  return (
    <form onSubmit={onSubmit} className="mx-auto max-w-2xl space-y-4 px-5 py-10">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <input className="field" type="number" min="0" placeholder="Price (NGN)" value={form.price} onChange={(e) => set("price", e.target.value)} />
        <select className="field" value={form.city} onChange={(e) => set("city", e.target.value)}>
          {CITIES.map((c) => <option key={c}>{c}</option>)}
        </select>
      </div>
      <input className="field" placeholder="Neighbourhood (optional)" value={form.neighborhood} onChange={(e) => set("neighborhood", e.target.value)} />
      <input className="field" placeholder="Image URL (optional)" value={form.image} onChange={(e) => set("image", e.target.value)} />
      <textarea className="field min-h-36" placeholder="Honest description: wear, size, pickup notes" value={form.description} onChange={(e) => set("description", e.target.value)} />
