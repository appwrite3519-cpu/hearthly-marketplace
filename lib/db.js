import { getServiceClient, isDatabaseConfigured } from "./supabaseServer";
...
export async function registerUser({ name, email, phone, city, password }) {
  const db = getServiceClient();
  const cleanEmail = String(email || "").trim().toLowerCase();
  const { data: existing } = await db.from("users").select("id").eq("email", cleanEmail).maybeSingle();
